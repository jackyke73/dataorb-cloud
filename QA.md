...
## External review follow-up — September 11, 2026
Acted on an external design-strategy review. Verified each technical claim in the browser before changing anything; the four measurable ones were correct.

Fixed:
- **The primary call to action was unreadable, and had been since the redesign.** `#primary-nav a` sets `color: var(--ink)` at specificity (1,0,1), which beats `.button-accent`'s `color: #fff` at (0,1,0), so the navigation "Request a quote" button rendered near-black on deep blue at **2.16:1**. The hero button was unaffected at 8.42:1, which is why it survived earlier visual passes. The colour rule is now scoped with `:not(.button)`; measured 8.42:1 after the fix.
- **`--muted` failed on its own tinted surfaces.** #6b7684 measured 4.62:1 on white but 4.30:1 on `--surface` and 4.00:1 on `--surface-2`. Changed to #5c6775: 5.75 / 5.35 / 4.99.
- **Footer small print** was 3.94:1 on `--ink`; changed to #8590a0 at 5.62:1.
- **Focus indicator was invisible on dark surfaces.** The global focus outline uses `--accent`, which is 2.16:1 against `--ink`. Footer, utility bar and capability band now take a white outline.
- **The mobile table stopped being a table.** `display: block` on table elements strips their implicit ARIA roles, so below 760 px the GPU and process tables exposed no rows or cells. Explicit `role` attributes now survive the display change: 33 cells and 9 rows present at both 1440 px and mobile. `thead` stays `display: none` on mobile by design, so each cell carries its own visible label instead of a column-header association; adding the header row back would double the announcement.
- **Conditional fields were far from their trigger and announced nothing.** Hosting and networking now sit directly below Project type — measured gap from 630 px on mobile and 379 px on desktop down to **107 px at both widths**. A reveal now announces through a dedicated `role="status"` region. That region is deliberately separate from `#form-status`: the generic `change` handler calls `invalidateDraft()` after `syncProjectFields()`, so a message written to `#form-status` would have been cleared immediately. Verified the announcement survives, stays silent on initial load, clears on hide, and does not steal focus.
- **`<summary>` used `display: flex`,** which is known to drop the disclosure mapping in Safari/VoiceOver. Layout moved to an inner `.faq-row` span so summary keeps its native `display: list-item`. The specifications disclosure and the FAQ now share one marker treatment across all seven disclosures instead of two unrelated affordances.
- **Control labels did not describe their behaviour.** The GPU table action said "Select" with an aria-label promising a quote request, when it fills the brief's GPU field and nothing is sent; it now reads "Add to brief" with a matching label. "Discuss Your Cluster" was the only Title Case control on the site and implied a conversation begins; it now reads "Prepare a cluster brief". Rental terms in the hero panel now say "specified per request" so the range does not read as an available product term.

Not done, and why. The review also proposed reordering the homepage, adding a "What you receive" section with deliverable samples, replacing the cluster grid with a responsibility matrix, and adding a "Who you work with" section. Those depend on business facts that cannot be invented: how Dataorb is paid and what "independent" means commercially, who contracts and invoices, which deliverables can already be produced to a repeatable standard, and which named people may be published. The review itself lists these as open questions. They remain open.

Verification limits unchanged from the review itself: no real Safari/VoiceOver, Firefox/NVDA or iOS testing, no 200%/400% zoom or forced-colours testing, and no Lighthouse or axe runs. The Safari disclosure change is a known-risk mitigation, not a verified fix on that platform.

Regression after all changes: no console errors, navigation CTA 8.42:1, table action still wires the GPU field, empty submit still raises seven field errors and states nothing was sent, cluster CTA still sets the project type and reveals the fields, draft correct with no `undefined`, stale answers still cleared on project-type change, helper-text association intact, seven disclosures operable, one H1, no delivery claim in visible text, and no horizontal overflow or navigation wrapping at 375, 500, 640, 768, 820, 900, 1000, 1010, 1100, 1280, 1440 or 1920 px.
